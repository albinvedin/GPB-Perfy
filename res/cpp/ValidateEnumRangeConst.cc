...
pgv::EnumRangeConst createMessage(
    int messageLength
){
  auto message = pgv::EnumRangeConst();
  for (int i = 0; i < messageLength; ++i) {
    message.add_content(pgv::EnumRangeConst_Sex::EnumRangeConst_Sex_MALE);
  }
  return message;
}

std::int64_t validateOne(
    pgv::EnumRangeConst const& message
