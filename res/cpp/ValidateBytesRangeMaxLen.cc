...
pgv::BytesRangeMaxLen createMessage(
    int messageLength
){
  auto message = pgv::BytesRangeMaxLen();
  for (int i = 0; i < messageLength; ++i) {
    message.add_content("\x99\x99\x99\x99\x99\x99\x99\x99\x99\x99");
  }
  return message;
}

std::int64_t validateOne(
    pgv::BytesRangeMaxLen const& message
